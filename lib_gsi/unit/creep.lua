		...
		if workingSet.dataThrottle:allowed() then
			...
			for hUnit,safeUnit in pairs(t_creeps) do
				...
				elseif hUnit:CanBeSeen() then
					...
					if not (newLoc.x == 0 and newLoc.y == 0 and newLoc.z == 0) then -- happens, not sure why, but it can cause TPS to mid. other weird behaviour
...
function cUnit_UpdateHealthAndLocation(gsiCreep)
	...
	gsiCreep.attackPointPercent = hUnitCreep:GetAttackPoint() / hUnitCreep:GetAttackSpeed()
end
...
function cUnit_GetCreepType(name)
	if string.find(name, "sieg") then
		return CREEP_TYPE_SIEGE
	elseif string.find(name, "mele") then
...
function cUnit_NewSafeUnit(hUnit)
	...
	newSafeUnit.isRanged = hUnit:GetAttackRange() >= CONSIDER_RANGED_ATTACK_MIN
	...
	if newSafeUnit.creepType == CREEP_TYPE_SIEGE then
		newSafeUnit.attackPointPercent = newSafeUnit.attackPointPercent + SIEGE_CREEP_LATE_RELEASE_BEHIND_ADJUST
	elseif newSafeUnit.creepType == CREEP_TYPE_RANGED then
		newSafeUnit.attackPointPercent = newSafeUnit.attackPointPercent + RANGE_CREEP_DIFFERENCE_IN_RELEASE_IN_PERCENT
	end
