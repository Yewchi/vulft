...
XETA_PUSH = function(p, t)
		...
		return t.goldBounty*max(0.33, (dpsToBuilding*2)/t.lastSeenHealth) -- safety checks are upper job
	end
...
--[[DEV]]if TEST then TEBUG_print(string.format("[xeta] Abscond values '%s': %.4f, %.4f, %.4f.", name, thisScoreTbl[1], thisScoreTbl[2], thisScoreTbl[3])) end
	end
