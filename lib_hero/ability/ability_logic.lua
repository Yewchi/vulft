...
function AbilityLogic_TryConfirmValidSkillBuild(gsiPlayer, skillBuildsTbl)
	...
	for i=1,#skillBuild,1 do
		...
		if not AbilityLogic_IsLikelyAbilityPointInvestibleType(thisAbility) then
			--[[DEBUG]]if DEBUG then DEBUG_print(string.format("ability_logic: %s skill build failed on check for %s. Trainable:%s. Upgradable: %s. <31:%s. >=0:%s.", gsiPlayer.shortName, thisAbility and thisAbility:GetName(), thisAbility and thisAbility:IsTrained(), thisAbility and thisAbility:CanAbilityBeUpgraded(), thisAbility and thisAbility and thisAbility:GetHeroLevelRequiredToUpgrade() < 31, thisAbility and thisAbility:GetHeroLevelRequiredToUpgrade() >= 0)) end
