...
d = {
	...
	["Initialize"] = function(gsiPlayer)
		...
		SpecialBehavior_RegisterBehavior("fightHarassRunOverride",
				function(gsiPlayer, objective, score)
					...
					--[[DEV]]if VERBOSE then print("'[hero_jugg] special 2'", bladeFury and bladeFury:GetName(),  hUnit:HasModifier("modifier_juggernaut_blade_fury")) end
					if not bladeFury 
					...
					--[[DEV]]if VERBOSE then INFO_print("jug bladfury radius %s fht crowdedLoc %s fhtLoc %s rating %.3f", bladeFuryRadius, crowdedLoc, fhtLoc, crowdedRating) end
					if crowdedRating > 1.1 then
						--[[DEV]]if VERBOSE then INFO_print("jug moves to crowdedLoc") end
					...
					--[[DEV]]if VERBOSE then INFO_print("jug moves for spin to %s with fht at %s limit %.3f", moveTo, fhtLoc, limit or -0) end
	...
	["AbilityThink"] = function(gsiPlayer) 
		...
		--[[DEV]]if VERBOSE then print("jug bladeFuryRadius", bladeFuryRadius) end
		...
		--[[DEV]]if DEBUG and GameTime() % 4 < 0.2 then DEBUG_print("jugg bladeFuryDmg=%s", bladeFuryDamage) end
		...
		if not inBladeFury then
			if bladeFuryCanCast then
				...
				if hasIncomingImmunablePj then
					--[[DEV]]if DEBUG then INFO_print("immunable projectile jugg") end
				...
				if #allE > 0 and (currActivityType >= A_T.FEAR and futureDamage > 0
					...
					--[[DEV]]if DEBUG then INFO_print("jug bladefury rooted %s slow %s %s", hUnit:IsRooted(), gsiPlayer.currentMovementSpeed, hUnit:GetBaseMovementSpeed()) end
					USE_ABILITY(gsiPlayer, bladeFury, nil, 400)
					return;
				end
			end
			if currActivityType <= A_T.CONTROLLED_AGGRESSION
					and fhtReal then
				--[[DEV]]if DEBUG then INFO_print("jug checks aggressive bladefury %s > %s", bladeFuryRadius+88, distToFht) end
				if bladeFuryRadius+44 > distToFht
						and HIGH_USE(gsiPlayer, bladeFury, highUse, fhtPercHp) then
					--[[DEV]]if DEBUG then INFO_print("jug aggressive bladefury range dist to %s", fht) end
