...
d = {
	...
	["AbilityThink"] = function(gsiPlayer) 
		...
		--[[DEV]]if TEST then print("power shot ", powerShotRange) end
		...
		--[[DEV]]if TEST then print(powerShotDamage) end
		...
		if allEnemies[1] and CAN_BE_CAST(gsiPlayer, powerShot) then
			...
			--[[DEV]]if TEST then TEBUG_print(string.format("wind 2 %s, %.2f, %s, %d, %d", Util_Printable(target), dmgFactor, isKill, target and target.lastSeenHealth or -0, #allCastSucceedsUnits)) end
			if target and isKill and powerShot:GetManaCost() < gsiPlayer.hUnit:GetMana()
				...
				--[[DEV]]if TEST then TEBUG_print("wind 3 cast kill ps") end
		...
		--[[DEV]]if TEST then TEBUG_print(string.format("wind 5 %d", castSucceedsUnitsNearby and #castSucceedsUnitsNearby or 0)) end
		...
		--[[DEV]]if TEST then print(fhtReal, CAN_BE_CAST(gsiPlayer, shackleShot), castSucceedsUnitsNearby and castSucceedsUnitsNearby[1], gsiPlayer.lastSeenMana, shackleShot:GetManaCost()) end
		...
		if fhtReal and CAN_BE_CAST(gsiPlayer, shackleShot) and castSucceedsUnitsNearby
			...
			if bestTarget and bestHitCount >= 2 then
				...
				--[[DEV]]if TEST then print("wind 5.3", bestScoreShackle, Util_Printable(bestTarget), bestHitCount, abscondScore, HIGH_USE(gsiPlayer, shackleShot, highUse, (1-abscondScore*0.576)*(0.66 + (1-heatPerc)*0.66)), (1-abscondScore*0.576)*(0.66 + (1-heatPerc)*0.66), heatPerc) end
				...
				if abscondScore > 0.45 - heatPerc*0.2
					...
					--[[DEV]]if TEST then print("damn") end
				...
				--[[DEV]]if TEST then print("do other stuff") end
		...
		if castSucceedsUnitsNearby and castSucceedsUnitsNearby[1] then
			...
			if currActivityType <= ACTIVITY_TYPE.CONTROLLED_AGGRESSION then
				--[[DEV]]if TEST then print("wind 5.5") end
				if gsiPlayer.hUnit:HasModifier("modifier_windrunner_windrun") then
					...
					if fhtReal and not fht.hUnit:HasModifier("modifier_windrunner_windrun_slow") then
						--[[DEV]]if TEST then TEBUG_print("wind 6 windrun short attack range unslowed enemy") end
						pUnit_SetFalsifyAttackRange(gsiPlayer, windrunDebuffRadius-20)
					end
				end
						
				-- Shackle targets intended to combo
				--[[DEV]]if TEST then print("wind 6.5", fhtReal, not bestPossibleShackleFound, distToFht, shackleCastRange+SHACKLE_CONE_RANGE, CAST_SUCCEEDS(gsiPlayer, fht, shackleShot), HIGH_USE(gsiPlayer, shackleShot, highUse, fhtPercHp)) end
				if fhtReal and CAN_BE_CAST(gsiPlayer, shackleShot) 
					...
					--[[DEV]]if TEST then print("wind 6.52", enemyCreepSetNearby) end
					...
					--[[DEV]]if TEST then print("wind 6.6", score, Util_Printable(target), hitsLoc, satisfiesArgs) end
					if target then
						...
						if abscondScore > 0.4 and HIGH_USE(gsiPlayer, shackleShot, highUse, (1-abscondScore*0.576)*(0.66 + fhtPercHp*0.33)) then
							--[[DEV]]if TEST then print("damn creep abs scone", abscondScore) end
							t_shackle_expire_before_cast[gsiPlayer.nOnTeam] = GameTime() + SHACKLE_USE_EXPIRY
							USE_ABILITY(gsiPlayer, shackleShot, target, 400, nil)
							return;
						end
					else
						--[[DEV]]if TEST then TEBUG_print("wind 7 shackle") end
		...
		--[[DEV]]if TEST then print("wind 7.8", dam, high) end
