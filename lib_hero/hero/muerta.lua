...
--[[DEV]]if TEST then TEBUG_print(string.format("SHOT LOC %s %.4f %s %s", Util_Printable(thisBLoc), timeGivenB, Util_Printable(contact.shortName), Util_Printable(contact.lastSeen.location))) end
...
--[[DEV]]	if TEST and VERBOSE then TEBUG_print(string.format(":muerta: scored contact hero %s %.3f. Intented target on vector: [%s]", testContactUnit.shortName, thisScore, wouldAimAtTarget)) end
		end
	end
...
--[[DEV]]if TEST then TEBUG_print(string.format(":muerta: checking some of %d-size tree table for %s shot. care of waste is %.2f", numTrees, target.shortName, careOfWaste)) end
	while iTree <= numTrees do -- lua
		...
		--[[DEV]]if TEST then	print("Check tree", trees[iTree], treeLoc) end
				...
				if POINT_DISTANCE(impactLoc, treeLoc) < ricochetTravelDist-50 then
					...
					if encased then
						...
						if treeScore > bestScore then
							...
							bestShot = thisTree
							bestIsAimedAtTarget = true
			--[[DEV]]		--DebugDrawLine(playerLoc, treeLoc, 0, 0, 255)
			--[[DEV]]	else
			--[[DEV]]		--DebugDrawLine(playerLoc, treeLoc, 255, 0, 0)
						end
			--[[DEV]]	if TEST and VERBOSE then
			--[[DEV]]		TEBUG_print(string.format(":muerta: scored tree %d %.3f.", thisTree, treeScore))
			--[[DEV]]	end
					end
				end
			end
...
--[[DEV]]if TEST then TEBUG_print(string.format("Muerta test shot value %.2f at via %s with careOfWaste %.2f. Score abscond rating: %.3f", bestScore, Util_Printable(bestShot), careOfWaste, ABSCOND_SCORE("muertaDeadShot", bestScore))) end

--[[DEV]]print(ABSCOND_SCORE("muertaDeadShot", bestScore),ABSCOND_SCORE("muertaDeadShot", bestScore))
...
--[[DEV]]if TEST then TEBUG_print(string.format(":muerta: returning shot %s", Util_PrintableTable(bestShot))) end
		return bestShot
	end

	if shotKills and distToTarget < deadShot:GetCastRange() * 0.95 then
--[[DEV]]if TEST then TEBUG_print(string.format(":muerta: returning safe insta-kill %s", Util_PrintableTable(bestShot))) end
...
d = {
	...
	--[[DEV]]	if TEST then TEBUG_print(string.format(":muerta: crowded rating for The Calling at enemy %s is %.2f",
	--[[DEV]]			fht.shortName, crowdedRating)) end
		...
		--[[DEV]]if TEST then DebugDrawText(700+(TEAM_IS_RADIANT and 0 or 100), 650, string.format("mu encase %d", #enemiesInRect), 255, 255, 255) end
		...
		damageInTimeline = damageInTimeline or DAMAGE_IN_TIMELINE(gsiPlayer.hUnit)
		--[[DEV]]print("muerta dmg in timeline", damageInTimeline, DAMAGE_IN_TIMELINE(gsiPlayer.hUnit))
		if CAN_BE_CAST(gsiPlayer, pierce)
			...
			--[[DEV]]print("muerta check danger dmg timeline", damageInTimeline)
